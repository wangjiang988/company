            ...
            _difffun = function(){
                ...
                if (_length == 2) {
                     _timeWapper.find("span") 
                                    .eq(0).text(parseInt(_minite)).end()
                                    .eq(1).text(_second)
                }
                else if (_length == 3) {
                     _timeWapper.find("span") 
                    ...
                    var _day = _totalsecond / 60 / 60/ 24
                    _hours = parseInt(_hours) % 24
                    _minite = parseInt(_minite) % 60
                    _second = parseInt(_second) % 60
                    _hours = _hours.toString().length == 2 ? _hours : "0" + _hours
                    _minite = _minite.toString().length == 2 ? _minite : "0" + _minite
                    _second = _second.toString().length == 2 ? _second : "0" + _second
                    _timeWapper.find("span").
                                     eq(1).text(parseInt(_day)).end()
                                    .eq(3).text(_hours.toString().slice(0, 1)).end()
                                    .eq(4).text(_hours.toString().slice(1)).end()
                                    .eq(6).text(_minite.toString().slice(0, 1)).end()
                                    .eq(7).text(_minite.toString().slice(1)).end()
                                    .eq(9).text(_second.toString().slice(0, 1)).end()
                                    .eq(10).text(_second.toString().slice(1))
                }
                else if (_length >5) {
                    _timeWapper.find("span").eq(0).text(_hours.slice(1) > 9 ? "" : _hours.slice(0, 1)).end()
