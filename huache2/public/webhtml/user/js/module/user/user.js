...
define(function (require, exports, module) {
    ...
    var vm = avalon.define({
        ...
        check_other_info_member:function(){
            var _title = $("input[name='title']:checked").val();
            var _other_title = $("input[name='other_title']").val();
            var _province = $("input[name='province']").val();
            var _city  = $("input[name='city']").val();
            var _address = $("textarea[name='address']").val();
            if( (_title=='other' && _other_title=='') || _title == null){
                alert('您的称谓输入不对，请重新填写');
                return false
            }
            if( _province ==''  || _city == null){
                alert('您的常用地市还未进行选择省市，请选择后再提交');
                return false
            }
            if(_address==''){
                alert('请填写您的详细地址');
            ...
            $("form[name='otherInfoForm']").submit();
            
        }
        ,
        edit_member_other_info:function(){
            $("form[name='otherInfoForm']").toggle();
            $("#mytitle").toggle();
            $("#myaddress").toggle();
            $("#sub_but").toggle();
        }
        ...
        SendCode: function (phone) {
            ...
            $.getJSON("/sendcodechangephone/"+ phone+"/1", function (data) {
        ...
        ,
        SendEmailToChangePwd: function () {
            var _form = $("form[name='passwd_form']")
            var _code = $("input[name='code']")
            var check_code_url = '/user/checkcode?t='+ Math.random();
            var _url = check_code_url+'&captcha='+_code.val()
            if ($.trim(_code.val())!="") {
                $.ajax({  
                     type:'get',  
                     url : _url,  
       ...
       ,
        ...
        ,
        GetCode:function(){
            var _code = $("input[name='phone']")
            if ($.trim(_code.val())!="") {
                var _this = $(this)//发送按钮本身 
                $.getJSON("/sendcodechangephone/"+ _code.val()+"/2", function (data) {
                    var _error_code = data.error_code
                    var _error_msg = data.error_msg
                    //console.log(data)
                    if (_error_code == 1) {
        ...
        modifyPhone:function(){
            ...
            if ($.trim(_phone.val())=="" || telReg==false){
                ...
                if ($.trim(_code.val())!="") {
                    $.get("/getcode/", function (data) {
                        if (data == _code.val() ) {
                            $("form[name='change_phone_form']").submit();
                            //发送邮件
                        } else{
                            vm.showerror()
                            _code.focus()
        ...
        SubmitValite:function(){
            ...
            var _userCard = $("input[name='bank_account']")
        ...
        SubmitCardInfo:function(){
            ...
            $.each($("input[type='file']"),function(index,item){
                var _img = $(item).prev().attr('src');
                var _img_exist = false;
                if(_img!="/themes/images/user/cd-0.gif" || _img!="/themes/images/user/cd-1.gif"){
                    _img_exist = true;
                }
                if ($(item).val()=="" && _img_exist == false) {
                    $(item).next().next().removeClass('hide').show()
                    vm.showHideError($(item).next().next())
                    _flag = false
                }
            ...
            if (_flag) {
                var bankCardForm = $("form[name='bankCardForm']")
                bankCardForm.submit();return false
                var options = {
        ...
        ,
        selectOrderTimeChoose:function(val){
            ...
            window.location.href="/user/memberOrder?date="+val;
        }
        ,
        ...
        ,
        selectDumpList:function(val){
                $(this).parent().parent().next().val(val)
                $(this).parent().parent().prev().find(".dropdown-label span").text($(this).text())
                $(this).parent().parent().toggle().find(".dl").toggle();
        }
        ,
        selectDumpList2:function(val){
            $(this).parent().parent().next().val(val)
            $(this).parent().parent().prev().find(".dropdown-label div").text($(this).text())
            $(this).parent().parent().toggle().find(".dl").toggle();
        }
        ,
        billing:function(){
            ...
            var _hffp = _billingform.find("input[name='inv_title']")
            ...
            var _invoicetype = _billingform.find("input[name='invoice_type']")
            ...
            if ($.trim(_hffp.val())=="") {
                ...
                vm.showHideError(_inputerror) 
                _flag = false
            } else if(parseInt(_invoicetype.val()) == 2){
        ...
        ,
        confirmReceipt:function(order_num,ori){
            var _this = $(this)
            _this.text("已收到").addClass("oksure")
            $.get("/user/memberInvoice/"+order_num+"?receive="+ori,function(data){
                if (data=='ok') {
                    alert("已结收到，平台响应成功");
                    vm.confirmReceipt=function(){};
                }else{
                    alert("已结收到，平台响应不成功，请重试再试");
                }
            })
        }
        ,
        ...
        selectIdendityCate:function(index){
            ...
            if (index == 1) {
                if ($(this).prop("checked")) {
                    $('input[name=typetype][value=local]').prop("checked",false)
                ...
                if ($(this).prop("checked")) {
                    $('input[name=typetype][value=other]').prop("checked",false)
                    _othertype.prop("checked",false)
                     $(".select-city-cur").removeClass('select-city-cur')
                     $(".city-select").find("input[type='hidden']").val("")
                }
            }
