define(function(require, exports, module) {
  ...
  $(".dropdown-vehicular-applications li").click(function() {
      ...
      if (_index == 0) {
        ...
        var _this = $(this)
        $("#shangpai").hide()
        $(".tbl-file").slice(2,5).hide()
        $.ajax({
          url: '/dealer/ajaxCustfile/filelist',
          type: 'post',
          dataType: 'json',
          data: {
                type_id: _this.attr("data-id"),
  ...
  $(function() {
    $(document).delegate('a.use-occasions-add-file', 'click', function(event) {
      var car_use_type = $(".dropdown-vehicular-applications li.active").attr("data-id")
      var shenfen_id = $(".dropdown-categories li.active").attr("data-id")
      ...
      else if (_passenger.eq(1).find("input[type='hidden']").val().trim() == "" &&_passenger.eq(0).find(".dropdown-vehicular-applications li.active").attr("data-id")!="2") {
          _selectFlag = false
