define(function (require,exports,module) {
	...
	$(".goods-modify-link").click(function(){
		...
		$(".def").hide()
		$(".goods-control").show()
		$(this).parent().prev().find(".loading").show().end().find(".handler-loading").hide()
	...
	$(".goods-save-link").click(function(){
		$(".empty-error").hide()
		var xzj = $(".loading input[name^='xzj']")
		var prama = []; 
		$.each(xzj, function(index,item){
              if ($(item).prop("checked")) {
              	var _id 	= $(item).attr("name")
              	var _count  = $(item).parents("tr").find("td:last").prev().find("input[type='text']").val()
              	prama.push(
              			{
              				xzj_id:_id.replace(/[^0-9]/ig,""),
              				num:_count
              			}
              		) 
              }
		...
		if (prama.length == 0 || (prama.length > 0 && typeof(prama[0].num) == "undefined") ) {
			$(".empty-error").show()
			return
		}

		$.ajax({
			url: '/dealer/baojia/ajaxsubmit/original-save',
			type: 'Post',
			dataType: 'json',
			data: {
				prama: prama,
				_token: $("input[name='_token']").val(),
				bj_id:  $("input[name='bj_id']").val(),
			},
		})
		.done(function(data) {
	...
	//2017-7-4 by aliks #270 bug
	$(".next,.prev").on("click",function(event) {
		var _this = $(this)
		if (_this.hasClass('disabled-click')) return
		var _input = _this.prev()[0] ? _this.prev() : _this.next()
		var _count = parseFloat(_input.val())
		var _price = parseFloat(_input.attr("data-price"))
		var _txt = _this.parents("td.count").next().find("span")
		_txt.text(parseFloat(_count * _price).formatMoney())
	});
