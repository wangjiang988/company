define(function (require,exports,module) { 
	...
	// $("#time-span").text(_curtime.getFullYear() + "-" + (_curtime.getMonth() + 1 < 10 ? ("0" + (_curtime.getMonth() + 1)) : (_curtime.getMonth() + 1)) + "-" + _curtime.getDate() + " 周" + week[_curtime.getDay()] )
	//
	...
	_checkbox.each(function(index,item){
		if ($(item).prop("checked")) {
			...
			else if (_day == index+1) {
				_isworkday = !0
			}
		}
	})
	// _isworkday ?
	// 			$("#time-span").append("<span class='green ml20'>工作日<span>")
	// 		   :
	// 		   $("#time-span").append("<span class='red ml20'>非工作日<span>")
	/*if (_day == index+1) {
				_isworkday = !0
				$("#time-span").append("<span class='green ml20'>工作日<span>")
			}else
...
*/
	...
	$(".reset").click(function(){
		$(this).prevAll(".btn-group").find("input[type='text']").attr("placeholder","0000-00-00").val("0000-00-00")
	})
	...
	$(".modify-work").click(function(){
		...
		$.each($(".time-dlp-long"),function(index,item){
			...
			$(".no-offer_"+_index).find("input[type='text']").eq(index%2).val($(this).text() == "0000-00-00" ? "" : $(this).text())
		})
	...
	$(".cancel").click(function(){
        ...
        $("#cancle_index").val(index)
        console.log(index)
		$('#cancel_pop').hcPopup({'width':'450'});
	})
    ...
    $("#cancel_do").click(function(){
        ...
        $.ajax({
            type: "post",
            url: '/dealer/cancel_rest_day',
            data: {
                index: $("input[name='index']").val(),
                daili_dealer_id:$("input[name='daili_dealer_id']").val(),
				id:$("input[name='work_time_id']").val(),
