    ...
    <div class="container m-t-86 pos-rlt" id="vue">
        <div class="wapper">
            <div class="hd reg-box">
                ...
                {!! Form::open(['url'=>route('answer.verify'),'id'=>'password-step-3','role'=>'form']) !!}
                    {!! Form::hidden('email',$name) !!}
                    <div class="form" v-show="errorCount!=10">
                        ...
                        <table v-cloak class="reg-form-tbl wauto">
                            <tr class="text-gray">
                                ...
                                <td>
                                   <div class="psr">
                                     <input @keydown.13="send" @focus="resetPhoneError" maxlength="4" v-model="phone" :value="phone" type="text" name="phone_four" :class="{'card-last':true, ml10:true,'error-bg':phone=='' && isSendLoading}"  placeholder="4位数字~" />
                                     <span style="right:-210px;top:0;" class="ml20 red psa" v-show="isPhone" v-cloak>请输入注册手机号最后4位~</span>
                                   </div>
                                </td>
                            </tr>
                            @if($isOrder >0)
                            <tr class="text-gray">
                                ...
                                <td width="">
                                   <div class="psr">
                                     ...
                                     <span class="ml20 red psa wp100 buy-cx-error" v-show="isSelectModel" v-cloak style="right:-376px">请选择您购买过的一个车型~</span>
                                     <span class="ml20 gray psa wp100 buy-cx-error2" v-show="!isSelectModel && modelId" v-cloak>厂商指导价：@{{price}}</span>
                                     <div class="clear"></div>
                            ...
                            <tr class="text-gray">
                                ...
                                <td>
                                    <div class="psr">
                                   <input @keydown.13="send" @focus="resetInputError" maxlength="4" v-model="cardNum" name="id_cart" :value="cardNum" type="text" :class="{'card-last':true, ml10:true,'error-bg':cardNum=='' && isSendLoading}"  placeholder="4位数字或字母，字母请大写~" />
                                   <span style="right:-210px;top:0;" class="ml20 red psa" v-show="isInput" v-cloak>请输入您身份证号最后4位~</span>
                                   </div>
                                </td>
                            </tr>
                            @endif
                            <tr class="text-gray">
                              <td  colspan="2">
                                 <p class="fs12 tip-wx tac">温馨提示：为了您的账户安全，验证账户问题须全部答对，方能重置密码哦～</p>
                              </td>
                            </tr>
                        </table>
                       ...
                       <table v-show="errorCount == 10"  v-cloak class="reg-form-tbl w620">
                            <tr>
                                ...
                                <td class="text-gray"><br>
                                     <p>因今日申请邮箱验证但验证失败的次数过多，邮箱{{ changeEmail($name) }}</p>
...
@section('js')
    <script type="text/javascript">
        seajs.use(["vendor/vue","module/pwd/password-step-3-2-2"],function(a,b,c){
            b.init("{{$start_date}}","{{$end_date}}");
            b.initValiteOption({{$isOrder}},{{$isUserVerify}});
            b.initUrl("{{route('mail.newReset') }}","{{ route('pwd.pwdOver') }}");
        })
    </script>
@endsection
