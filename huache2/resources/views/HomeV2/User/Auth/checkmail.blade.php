                ...
                <h1 class="success-title">
                    ...
                    <table class="mauto">
                        <tr>
                            ...
                            <td><input @focus="initCode" v-model="code" maxlength="6" placeholder="请输入" type="text" name="code" :class="{'form-control':true,'custom-control':true,'error-bg':codeError}"></td>
                            {{ csrf_field() }}
...
@section('js')
    <script type="text/javascript">
        seajs.use(["/webhtml/common/js/vendor/vue.min","/webhtml/user/js/module/user/user-email-step2", "/webhtml/user/js/module/common/common"],function(v,u,c){
            u.initEmail("{{ $user->email }}","{{ route('account.sendMail') }}","{{ route('account.reset',['type'=>'email']) }}",'{{ route('account.checkEmail') }}');
        })
    </script>
@endsection