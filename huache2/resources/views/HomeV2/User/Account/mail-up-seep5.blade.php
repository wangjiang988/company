                ...
                <h1 class="success-title">
                    <table class="mauto">
                        <tr>
                            ...
                            <td class="text-left"><span class="inline-block c72 fs14 ml10">已发送验证邮件至：{{ $user->email }}</span></td>
                        </tr>
                        <tr>
...
@section('js')
    <script type="text/javascript">
        seajs.use(["/webhtml/common/js/vendor/vue.min","/webhtml/user/js/module/user/user-email-step2", "/webhtml/user/js/module/common/common"],function(v,u,c){
            u.initEmail("{{ $user->email }}","{{ route('account.sendMail') }}","{{ route('upemail.success') }}","{{ route('email.save') }}");
        })
    </script>
@endsection