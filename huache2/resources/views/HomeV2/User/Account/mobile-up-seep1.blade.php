        ...
        <div class="content-wapper ">
            <div class="hd">
                <ul>
                    ...
                    <li><span>2</span><label>修改验证手机号</label></li>
            ...
            {!! Form::open(['url'=>route('member.checkSms'),'role'=>'form','name'=>'code-phone']) !!}
                <div class="ml160 mt60">
                    ...
                    <phone-code class="valite-phone-code" ref="phoneCode" phone="{{ $user->phone }}" sendurl="{{ route('member.sendSms') }}" sendtype="78530066" iscode="1" v-on:valite-code="getCode"></phone-code>
                    <div class="clear"></div>
                    <p :class="{inputerror:true, 'normal-warn':true, ml50:true, red:true, hide:!error}">验证码有误，请重新输入~</p>
                </div>
                {!! Form::hidden('phone',$user->phone) !!}
                {!! Form::hidden('template_code','78530066') !!}
                {!! Form::hidden('max',30) !!}
                <br><br>
                <p class="tac"><a @click="next" href="javascript:;" class="btn btn-s-md btn-danger fs16 btn-s w150">下一步</a></p>
                <div class="m-t-10"></div>
                @if($user->email)
                    <p class="tac"><a href="{{ route('upmobile.seep2') }}" class="juhuang">
                使用已验证邮箱修改验证手机号&gt;&gt;</a></p>
                @endif
            {!! Form::close() !!}
            <div class="m-t-10"></div>
