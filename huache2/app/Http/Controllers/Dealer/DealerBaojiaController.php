...
class DealerBaojiaController extends Controller
{
    ...
    public function getBaojiaInfo($type, $id, $step)
    {
        ...
        if ($id == 0 && $step == 1) {//新增报价
            ...
            $this->checkSeller($id);
            $baojia = HgBaojia::getBaojiaInfoAndScope($id);
            $baojia->bj_end_time = (int)$baojia->bj_end_time ;
            if (empty($baojia)) {
                die('该报价不存在');
            }
...
//    			$view['allOtherPriceStr'] = $allOtherPriceStr;
                ...
                //设置开始时间和结束时间范围
                $day_info    = $this->_parse_time_range();
                $view['day'] = $day_info;

                $bj_temp_internal_color = $baojia['bj_temp_internal_color']?unserialize($baojia['bj_temp_internal_color']):array();
                $view['button_type'] = 1;  //提交按钮类型
                if(count($bj_temp_internal_color)>1){
                    $view['button_type'] = 2;
                }
                //获取节能补贴值
...
//                    }
//                }
                $order_num         =   HgOrder::get_success_order_num_by_bjId($baojia['bj_id']);
                $view['order_num'] =   $order_num ? $order_num : 0;
                $view['sum_num']   =   (int)$view['order_num'] + (int)$baojia['bj_num'];
                $view['suicheInfo'] = HcVehicleToolsFiles::getAnnex($baojia['brand_id']);
                if ($type == 'view') {//查看
                    return view('dealer.ucenter.baojia_view_step6', $view);
                } elseif ($type == 'edit') {
                    $internal_color = unserialize($baojia['bj_temp_internal_color']);
     ...
     * 计算报价可用生效的开始时间和结束时间
     */
    private function _parse_time_range()
    {
        $now   = Carbon::now();
        ...
        if($now->hour < 16) {
           $day_info  = [
                'start_day'  =>  $today,
                'end_day'    =>   $today,
                'start_hour' =>  $now->lte(Carbon::parse(date('Y-m-d'))->addHour(9))?9:$now->hour+1,
                'end_hour'   =>  $now->hour<=10?10:$now->hour+1,
                'start_add'      => 0,
                'end_add'        => 0,
            ];
        }elseif($now->hour == 16){
    ...
    public function postBaojia($type, $id, $step)
    {
        if ($id == 0) {
            ...
            $baojiaInfo = HgBaojia::getBaojiaInfoAndScope($id);
            if (!$baojiaInfo) {
            ...
            if ($step == 1) {//编辑保存第一步  车型价格
                ...
                if ($this->request->input('fulltime') == 1) {
                    ...
                    //查看当前时间是否小于开始时间，否，改为等待生效
                    if((int)$data['bj_start_time'] > time()){
                        $data['bj_status']  = 6;
                    }
                }

                if($data['bj_start_time']>$data['bj_end_time']){
                        return response()->json(['msg'=>'开始时间不能大于结束时间']);
                }
                $ret =  HgBaojia::editBaojia($id, 6, $data);
                if($ret['error_code'] == 0 ){
                    $baojia_id_list  =  explode(',', $ret['bj_id']);
                    if(empty($baojia_id_list)){
                        $baojia_id_list = [$id];
                    }
                    //等待报价不算
                    if(isset($data['bj_status']) && $data['bj_status'] == 6){
                        $ret['to']  =  'effective';
                        $update = [
                            'bj_status_change_code' => '',
                            'bj_reason' => '等待生效',
                            'bj_status_change_time' => Carbon::now(),
                            'bj_status' => 6,
                        ];
                        HgBaojia::whereIn('bj_id', $baojia_id_list)->update($update);
                        return $ret;
                    }
                    //检查当前可提现余额是否不足，如果不足
                    $member_id  =   session('user.member_id');
                    $hc_daili_account = HcDailiAccount::getAccountInfo(['d_id' => $member_id ]);
                    if(!$hc_daili_account)
                    {
                        return [
                            'error_code' =>  0,
                    ...
                    if((float)$hc_daili_account['credit_line'] + (float)$hc_daili_account['avaliable_deposit'] <0)
                    {
                        $ret['to']  =  'suspensive';
                        $update = [
                        ...
                        //查看当前是否是上架时间。如果是上架时间，则立即上架，如果非上架时间，下架。
                        $baojia  = HgBaojia::with(['work_time'])->where('bj_id',$id)->firstOrFail();
                        if($baojia  && $this->_check_rest_time($baojia)){
                            $ret['to']  =  'suspensive';
                            $update = [
    ...
    public function getBaojiaList($type)
    {
        ...
        if ($type == 'suspensive') {
            ...
            $view['choose_to_down_num'] = HgBaojia::getBaojiaCountByChooseToDown($member_id);
            $result = HgBaojia::getDealer($member_id, $id);
            $view['dealers'] = $result->where('bj_step', '99')
               // ->where('bj_start_time', '<=', time())
                ->where('bj_end_time', '>', time())
            ...
            if ($this->request->ajax()) {
                ...
                if (!empty($view['state'])) {
                    if($suspensive_bj_reason =  $this->_parse_search_status($view['state']) )
                    {
                        $quest->where('bj_reason', '=', $suspensive_bj_reason);
                    }
                }
                $view['lists'] = $quest->select('hg_baojia.bj_serial','bj_status_change_code', 'gc_name','bj_status' ,'bj_reason', 'hg_baojia.bj_id',
                    'bj_end_time', 'hg_baojia_price.bj_lckp_price')
                    ->orderByRaw("case when bj_reason='主动暂时下架' then 1 " .
                        "  when bj_reason='资金池可提现余额不足' then 2 " .
                        " when bj_reason='非销售时间' then 3 end  asc ")
                    ->orderBy('bj_status_change_time','asc')
                    ->paginate(10);
                $view['type'] = 'search';
        ...
        if ($type == 'useless') {
            ...
            if ($this->request->has('timeout_reason')) {
                $seachParams['bj_final_reason'] = $this->request->input('timeout_reason');
            }
    ...
    private function  _parse_search_status($status)
    {
        $bj_reason  = '';
        switch($status)
        {
            case -1:  return false;
            case '0': $bj_reason='非销售时间';break;
            case '1': $bj_reason='主动暂时下架';break;
            case '2': $bj_reason="资金可提现余额不足";break;
    ...
    public function ajaxSubmit($type)
    {
        ...
        if ($type == 'import-insurance') {//保险导入功能
            ...
            $is_install = empty($this->request->input('is_install')) ? '0'
                : $this->request->input('is_install');
            ...
            if ($c >= 1) {
                return json_encode(array(
                    'error_code' => 1,
                    'message'    => '您添加的项目已经存在，不能重复~'
                ));
            } else {
                $zengpin = HgZengpin::find($zengpin_id);
                $data = array(
                    'bj_id'      => $bj_id,
                ...
                if ($e === false) {
                    return json_encode(array(
                        'error_code' => 1,
                        'message'    => '抱歉！提交失败，请重新提交~'
                    ));
                } else {
                    $data = HgBaojiaZengpin::join('hg_zengpin',
                        'hg_baojia_zengpin.zengpin_id', '=', 'hg_zengpin.id')
                        ->where('hg_baojia_zengpin.bj_id', $bj_id)
                        ->select('hg_zengpin.title as zp_title',
                            'hg_baojia_zengpin.id',
                            'hg_baojia_zengpin.zengpin_id',
                            'hg_baojia_zengpin.is_install',
                            'hg_baojia_zengpin.num')
                        ->get();
            ...
            $id = $this->request->input('id');
            $bj_id = $this->request->input('bj_id');
            $e = HgBaojiaZengpin::where('id', $id)->where('bj_id', $bj_id)
                ->delete();
            if ($e === false) {
                return json_encode(array(
                    'error_code' => 1,
                    'message'    => '抱歉！删除失败，请重新再试~'
                ));
            } else {
                return json_encode(array(
                    'error_code' => 0,
            ...
            //终止报价前，查看是否已经产生相应订单，如果有相应订单，则下架状态，改为 '已被订购'
            //如果没有，则下架状态改为  '主动终止'
            $baojia = HgBaojia::where('bj_id', $bj_id)->firstOrFail();
//
            $order_count   = HgOrder::get_success_order_num_by_bjId($bj_id);
            $baojia_reason = $order_count>0?'已被订购':'主动终止';
            if($baojia->bj_final_reason != '已被订购') $baojia->bj_final_reason = $baojia_reason;
            $result = $baojia->down_baojia($baojia_reason);


            if ($result == 1) {
                //询问是否下架有相同车架号的报价
                $other_list
                    = HgBaojia::getOtherBaojiaWithSameInternalId($baojia,
                    [1, 2]);
                $ids = [];
                $series = [];
                if($other_list){
                    $other_list->map(function ($item) use (&$ids, &$series) {
                        $ids[]    = $item->bj_id;
                        $series[] = $item->bj_serial;
                    });
                }
        ...
        } elseif ($type == 'suspensive_all-baojia') {
//            $bj_id = explode(',', $bj_id);

            $result = HgBaojia::down_all_baojia($bj_id, $member_id);
...
//            }
            ...
            $data = HgBaojia::with(['work_time'])->where(['m_id' => $member_id, 'bj_id' => $bj_id])
                ->firstOrFail();

            $dealer_count =   HcDailiAccount::getAccountInfo(['d_id'=>$member_id]);
            if(!$dealer_count){
                return [
                    'error_code' => 1,
                    'error_msg' => "找不到用户账户"
            ...
            //判断资金池用户可提现余额不足，
            if(!$this->_check_account_money($dealer_count)){
                //修改报价为可提现余额不足，
                HcDailiAccount::suspend_all_baojia_by_account($dealer_count);
                //修改当前报价原因为可提现余额不足
                $data->hwache_baojia_money_end();

                return [
                    'error_code' => 1,
                    'error_msg' => "因您的资金池可提现余额不足，报价{$data->bj_serial}扔处于暂时下架状态，请尽快增加您账户的余额。"
                ];
            }

             //判断是否非工作时间
            if($this->_check_rest_time($data)){
                //修改报价为非销售时间
                 $data->hwache_baojia_end();

                 return [
                    'error_code' => 1,
                    'error_msg' => "因不在您设定的工作时段内，报价{$data->bj_serial}扔处于暂时下架状态，下一个销售时间开始后将自动生效"
                ];   
            }  

            //判断是否提现   
            if ($data->bj_reason == '主动暂时下架') {
                $result = HgBaojia::where([
                    'm_id'  => $member_id,
                    'bj_id' => $bj_id
                ])
                    ->update(['bj_status' => 1]);
                if ($result == 1) {
                    return array('error_code' => 0,'error_msg' => "报价{$data->bj_serial}已恢复生效");    //js 中0为成功
                } else {
                    return array('error_code' => 1, 'error_msg' => '恢复失败');
                }
            } else {
                return array('error_code' => 1, 'error_msg' => '此报价不符合条件');
            }
        } elseif ($type == 'find-price') {
            $price = HgCarInfo::getCarmodelFields($bj_id);
            return json_encode($price);
        } elseif ($type == 'shelves-all-baojia') { //一键恢复所有已主动暂停报价
            //先判断是否余额不足
            $dealer_count =   HcDailiAccount::getAccountInfo(['d_id'=>$member_id]);
            if(!$dealer_count){
                return [
            ...
            $conditions = ['bj_reason' => '主动暂时下架', 'bj_status' => '2', 'm_id'=>$member_id];
            $list       = HgBaojia::with(['work_time'])
                         ->where($conditions)
                         ->select('bj_id','bj_serial','daili_dealer_id')->get();
            ...
            if(count($list)>0){
                ...
                foreach ($list as $item)
                {
                    ...
                    if($is_less_money)
                    {
                        $item->hwache_baojia_money_end();
                        continue;
                    }
                    //判断是否在销售时间或者余额是否充足
                    if(HgBaojia::check_rest_time($item)){
                        $item->hwache_baojia_end();
                        continue;
                    }

                    if($i == (count($list)-1))
                        $id_string .= $item['bj_serial'].'';
                    elseif($i%2==1)
                          $id_string .= $item['bj_serial'].'、<br/>';
                    else
                        $id_string .= $item['bj_serial'].'、';
                    $i++;
                }
            ...
            if($id_string == ''){
                return array('error_code' => 1, 'error_msg' => '暂无可恢复生效的报价');
            }
            
            $status = ['bj_status' => '1'];
            $result = HgBaojia::allOperation($member_id, $conditions, $status);
            if ($result) {
                return array('error_code' => 0, 'error_msg'=>'您已恢复下列报价：<br/>'.$id_string);    //js 中0为成功
            } else {
                return array('error_code' => 1, 'error_msg' => '操作失败');
            }
        } elseif ($type == 'ceaseves-all-baojia') {
            //一键暂停所有的报价
            $conditions = ['bj_status' => '1'];
            $status = ['bj_status' => '2', 'bj_reason' => '主动暂时下架'];
            $result = HgBaojia::allOperation($member_id, $conditions, $status);
            if ($result) {
                return array('error_code' => 0);    //js 中0为成功
            } else {
        ...
        } elseif ($type == 'original-save') {
            //获取前台数据
            $press = $this->request->input('prama');
            // 得到报价相关信息
            $bj_id = $this->request->input('bj_id');
            $baojia = HgBaojia::find($bj_id)->toArray();
            // 补充报价选装件数据,保存数据库
            $press_xzj_id = array_pluck($press, 'xzj_id');
            //先删再保存
            HgBaojiaXzj::where('bj_id',$bj_id)->delete();
            $dailiXzj = HgXzjDaili::getDealerXzjDatas(
                $baojia['brand_id'],
                $baojia['daili_dealer_id'],
                $baojia['car_staple_id'],
                $press_xzj_id
            );
            $arr = [];
            foreach ($press as $item) {
                $arr[$item['xzj_id']] = $item;
            }
            if ($dailiXzj->count()) {
                foreach ($dailiXzj as $k => $v) {
                    $bj_xzj[] = array(
                        ...
                        'is_install'  => 1,
                        'num'         => $arr[$v->xzj_list_id]['num'],
                        'fee'         => $v->xzj_fee,
                        'guide_price' => $v->xzj_guide_price,
                        'm_id'        => $v->id
                    );
                }
                if (HgBaojiaXzj::insert($bj_xzj)) {
     ...
     * 检查用户可用余额是否符合恢复规则
     */
    private function _check_account_money(HcDailiAccount $account)
    {
        //账户余额
        if($account->avaliable_deposit >0) return true;

        //可用额度+信用额度 扔小于0 则账户不可用、
        if((float)$account->avaliable_deposit + (float)$account->credit_line <=0)
        {
            return false;
        }
        return true;
     ...
     *
     */
    private function _check_rest_time(HgBaojia $baojia){
    ...
    {
        $result = HgBaojia::where('m_id',session('user.member_id'))
            ->find($id);
        if (is_null($result)) {
            die('不属于你的报价');
        }
