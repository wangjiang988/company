    ...
    public function getAjax(Request $request){
       ...
       //if($request ->ajax()){
        ...
        if(!Cache::has($cacheName)) {
            ...
            //是否现车
            $search['is_xianche'] = $is_xianche = Input::get('xian_che',0);
            $search['city'] = $city;
            ...
            */
            ...
            if (!empty($factory_date)){
                if($is_xianche==1)
                $map[] = $this->CcNy($factory_date);
            }

            //交车周期
            if (!empty($term)){
                if(empty($is_xianche) || $is_xianche==2)
                $map[] = 'bj_jc_period =' . $term . ' and bj_is_xianche =0';
            }
            ...
            if(!empty($is_xianche)){
                $bj_js_xianche = ($is_xianche==1) ? 1 : 0;
                $map[] = "bj_is_xianche = ".$bj_js_xianche;
            }
            //$map[] = "bj_id = 864";
            // 分页页码
            $page = intval(Input::get('page', 1));
            $pageSize = intval(Input::get('size', 10));
            // 查询报价列表，包含分页
            ...
            foreach ($BjResult['data'] as $v) {
               ...
               /* $bxTotal = Search::getBXTotal($carTag['seat_num'], $v['bj_id'], $carTag['guobie'], $is_job);
                ...
                //加密
                $text = $v['bj_id'].','.$city;
                $v['url'] = hc_encrypt($text, 'HwaChe@cn');
                //}
                $baojiaList[] = $v;
            }
