...
class Area extends Model {
     ...
     * @return mixed
     */
    public static function getAllTopArea() {
        $sql = self::select('area_id', 'area_name' ,'first_letter' )
            ->where('area_id', '<>', 35)
            ->where('area_parent_id', 0)
            ->where('area_deep', 1)
    ...
    {
        return $query->where('area_parent_id','=', 0)
            ->where('area_deep','=', 1)
     ...
     * @return mixed
     */
    public static function getSecAreaAll() {
        $cacheName = 'cacheSecArea';
        if(!Cache::has($cacheName)) {
            $cacheData = self::getAllTopArea()->toArray();
            foreach ($cacheData as $key => $value) {
                $cacheData[$key]['child'] = self::select('area_id', 'area_name' ,'area_xianpai')
                    ->where('area_parent_id', $value['area_id'])
                    ->where('area_deep', 2)
    ...
    {
        $city_list = self::where('area_xianpai',1)->where('area_deep', 2)->pluck('area_name', 'area_id');
        if ($city_list) {
    ...
    /** 通过名称查找id
     * @param $areaName
     * @param int $area_deep 默认查找城市
     * @return mixed
     */
    public static function getNameToId($areaName,$area_deep=2)
    {
        return self::where('area_name','like','%'.$areaName.'%')
            ->where('area_deep','=',$area_deep)
            ->value('area_id');
    }
}
