...
class HgBaojia extends Model {
    ...
    {
        return $this->belongsTo(HgDelaerWorkday::class, 'daili_dealer_id','daili_dealer_id');
    }
     ...
     * 价差是否是工作时间
     */
    public static function check_rest_time(HgBaojia $baojia)
    {
     ...
     * 车辆下架操作
     */
    public function down_baojia( $reason ="已被订购")
    {
        $this->bj_reason = $reason;
        $this->bj_status = 4;
        $ret = $this->save();

        //下架同一售方，同一车型，相同车辆编号的所有报价
        if($this->bj_dealer_internal_id != '')
        {
             HgBaojia::where('bj_dealer_internal_id',$this->bj_dealer_internal_id)
                    ->where('brand_id',$this->brand_id)
                    ->where('m_id', $this->m_id)
                    ->update(['bj_status'=>4, 'bj_reason'=>$reason]);
        }
        return $ret;

    }

    public static function down_all_baojia($bj_id, $member_id)
    {
        if(!$bj_id) return false;

        $q = "UPDATE car_hg_baojia SET bj_status=4, bj_reason='主动终止',bj_final_reason=" .
            "(case bj_final_reason when '已被订购' then '已被订购' else '主动终止' end)" .
            " where bj_id in (".$bj_id.") and m_id=".$member_id;
    ...
    public static function getBaojiaInfo($id, $serial = false) {
        $sql = self::leftJoin(
                            'hc_price as bp',
                            'hg_baojia.bj_id', '=', 'bp.id'
                            )
                    ->leftJoin(
                            'hg_baojia_area as ba',
                            'hg_baojia.bj_id','=','ba.bj_id'
                            );
        if ($serial) {
    ...
    /**
     * 获取与当前车报价有相同的“内部车辆编号“的 状态为正在发布的 其他报价
     * @param HgBaojia $baojia
     * @param array $status bj_status
     * @return array
     */
    public static function getOtherBaojiaWithSameInternalId(Hgbaojia $baojia, $status=[1])
    {
        $list = null;
        if ($baojia->bj_dealer_internal_id) {
            //锁定车源判断标准：A.同一个售方账户下的、B.同一个车型规格下的，C.内部车辆编号完全相同，A+B+C同时满足则锁定为同一组
            $dl_id = session('user.member_id');//代理ID

            $list = self::where('brand_id', $baojia->brand_id)
                ->where('bj_dealer_internal_id', $baojia->bj_dealer_internal_id)
                ->where('m_id', $dl_id)
                ->whereIn('bj_status', $status)//上架或暂停的数据都要
                ->where('bj_id', '!=', $baojia->bj_id)
                ->get(['bj_serial', 'bj_id']);
        }
     ...
     * @author  wangjiang
     */
    public static function set_hwache_baojia_end($member_id, $dealer_id,$daili_dealer_id)
    {
        $now  = Carbon::now();
        $update_down  = [
        ...
        self::where('m_id',$member_id)
                ->where('dealer_id', $dealer_id)
                ->where('daili_dealer_id', $daili_dealer_id)
                ->where('bj_reason', '!=', '主动暂时下架')
                ->update($update_down);
    }
     ...
     * @author  wangjiang
     */
    public static  function set_hwache_baojia_start($member_id, $dealer_id,$daili_dealer_id)
    {
        $now  = Carbon::now();
        $update_up  = [
            ...
            'bj_status_change_time' =>  $now,
            'bj_status'    =>  1,
            'bj_reason'    =>  "baojia:hourly 时间:{$now} 报价上架",
        ];

        self::where('m_id',$member_id)
            ->where('dealer_id', $dealer_id)
            ->where('daili_dealer_id', $daili_dealer_id)
            ->where('bj_status_change_code', 'hwache_baojia_end')  //其他下架理由不予上架
            ->update($update_up);
    }
     ...
     * @return mixed
     */
    public static function getBaojiaInfoAndScope($id, $serial = false)
    {
            ...
            try {
                ...
                if($step==1){//修改车型价格
                    ...
                    */
                    ...
                    //删除车身属性 老数据
                    $delete_where = $baojiaInfo['bj_is_xianche'] == 1 ? array('body_color', 'interior_color') : array('body_color');
                    ...
                    $carAttr = array();
                    $carAttr[] = array('bj_id'=>$bj_id,'name'=>'body_color','value'=>serialize($data['body_color']));
                    if($baojiaInfo['bj_is_xianche']==1){//现车  内饰提交
                        $carAttr[] = array('bj_id'=>$bj_id,'name'=>'interior_color','value'=>serialize($data['interior_color']));
                    }
...
//                    }
                   ...
                   if($xzj) {
                    ...
                    //列出常用管理的其他收费复制到报价表的其他收费中
                    if ($baojiaInfo['bj_step'] == 3) {
                        $other_price = HgDealerOtherPrice::where('daili_dealer_id',
                            $baojiaInfo['daili_dealer_id'])->get([
                            'other_id',
                            'other_name',
                            'other_price'
                        ]);
                        if ($other_price->count()) {
                            $other_price->each(function (&$item) use ($bj_id) {
                                $item->bj_id = $bj_id;
                            });
                            $data = $other_price->toArray();
                            HgBaojiaOtherPrice::insert($data);
                        }
                    }
                    $e = DB::table('hg_baojia')->where('bj_id',$bj_id)->update($carInfo);
                    if($e===false){
                    ...
                    **/
                    ...
                    $otherPriceTotal = 0;
                    //其他费用合计
                    $otherPriceTotal = DB::table('hg_baojia_other_price')->where('bj_id',$baojiaInfo['bj_id'])->sum('other_price');
                    ...
                    if($baojiaInfo['bj_is_xianche']==0){
                        ...
                        foreach($interior_color as $key=> $color)
                        {
                            $carAttr= array(
                                'name'=>'interior_color',
                                'value'=>serialize($color)
                            );

                             if($key > 0){
                                 $ret   =  self::copyBaojia($baojiaInfo['bj_id'], $member_id,true);
                                 if($ret['error_code'] != 0)
                                 {
                                     throw new Exception('数据提交失败[非现车内饰]【hg_baojia_field】');
                                 }
                                 $bj_id_list[] = $ret['bj_id'];
                                 $carAttr['bj_id'] = $ret['bj_id'];
                             }else{
                                 $carAttr['bj_id'] = $bj_id;
                             }
                        ...
                        if(isset($data['bj_status'])){
                            $carInfo['bj_status'] = $data['bj_status'];
                        }
                        //报价信息更新
//                        foreach ($bj_id_list as $bid){
                            $e = DB::table('hg_baojia')->whereIn('bj_id',$bj_id_list)->update($carInfo);
                            if($e===false){
...
//                        更新非现车中的数据 临时内饰
            ...
            if ($step == 6) {
                // 生成报价，调用价格公式接口
                if(!isset($bj_id_list)) $bj_id_list[] = $bj_id;
                foreach ( $bj_id_list as $bj_id_item)
                {
                    $accounting = app('App\\Com\\Hwache\\Baojia\\Accounting');
                    $hcPrice    = $accounting->getPriceAll($bj_id_item);
                    $accounting->savePrice($bj_id_item, $hcPrice['hcPrice']);
                    (new Shenhe())->verifyBaojia($bj_id_item);
                }
            }
            /**
             * End
            ...
            if(!isset($bj_id_list)){
             ...
             return array('error_code'=>0,'message'=>'数据保存成功','bj_id'=>implode(',', $bj_id_list));
     ...
     * @param 是否为拆分报价操作 如果拆分操作，颜色内饰不予操作
     * return
     */
    public static function copyBaojia($bj_id, $member_id, $is_split = false)
    {
        ...
        try {
            ...
            if ($BaojiaFields) {
                foreach ($BaojiaFields as $field) {
                    if($is_split && $field['name'] == 'interior_color') continue; //如果是拆分操作，这里不写颜色
                    unset($field['id']);
     ...
     * 非销售时间 对象  主动下架
     */
    public function hwache_baojia_end(){
        $this->bj_status_change_code   =  'hwache_baojia_end';
        $this->bj_reason               =  '非销售时间';
        $this->bj_status_change_time   =  Carbon::now();
        $this->bj_status               =  2;
     ...
     * 余额不足 对象  主动下架
     */
    public function hwache_baojia_money_end(){
        $this->bj_status_change_code   =  'hwache_baojia_money_end';
        $this->bj_reason               =  '资金池可提现余额不足';
        $this->bj_status_change_time   =  Carbon::now();
        $this->bj_status               =  2;
    ...
    public static function getBaojiaCount($member_id)
    {
        ...
        $count['effective'] = DB::table('hg_baojia')
                             ->where(['m_id'=>$member_id,'bj_step'=>99,'bj_status'=>6])
...
//                            })
    ...
    public static function getBaojiaCountByChooseToDown($member_id)
    {
        $count = DB::table('hg_baojia')
            ->where('m_id',$member_id)
            ->where('bj_step', '99')
            ->where('bj_status','2')
            ->where('bj_reason','主动暂时下架')
    ...
    public static function getWaiting($member_id)
    {
        ...
        $list = self::where(['m_id'=>$member_id,'bj_step'=>99,'bj_status'=>6])
                         ->where('bj_start_time','>',time())
    ...
    public static function getOnline($member_id)
    {
        $list = self::leftjoin('hg_baojia_price','hg_baojia.bj_id','=','hg_baojia_price.bj_id')
                 ...
                 ->where('bj_start_time','<=',time())
                 ->where('bj_end_time','>',time())
                 ->select('hg_baojia.bj_serial','gc_name','hg_baojia.bj_id','bj_end_time','hg_baojia_price.bj_lckp_price')
    ...
    public static function getSuspensive($member_id)
    {
        $list = self::leftjoin('hg_baojia_price','hg_baojia.bj_id','=','hg_baojia_price.bj_id')
                ...
                // ->where('bj_start_time','<=',time())
                 ->where('bj_end_time','>',time())
                 ->select('hg_baojia.bj_serial','gc_name','bj_reason','hg_baojia.bj_id','bj_end_time','bj_status','bj_status_change_code','hg_baojia_price.bj_lckp_price')
...
//                ->update($status);
    ...
    public static function getUselessBaojiaList($seachParams, $member_id, $start, $end)
    {
        ...
        $list = self::where($seachParams)
                    ->Where('bj_status','=',4)
                    ->select('bj_id','bj_serial','gc_name','bj_update_time','bj_reason','bj_final_reason')
                    ->orderBy('bj_update_time', 'desc')
                    ->paginate(10);

        if($list->count()>0)
        {
            $list->map(function($baojia){
                    $baojia->orders = HgOrder::where('bj_id',$baojia->bj_id)
    ...
    public function getBaojiaData($id)
    {
        $result = self::leftjoin('hc_price', 'hg_baojia.bj_id', '=', 'hc_price.id')
            ...
            ->leftjoin('hg_baojia_fields','hg_baojia_fields.bj_id','=','hg_baojia.bj_id')
            ->where('hg_baojia_fields.name','interior_color')
            ->find($id);
        if (count($result)) {
    ...
    public function baojiaxzj()
    {
        return $this->hasMany(HgBaojiaXzj::class,'bj_id','bj_id');
    }
