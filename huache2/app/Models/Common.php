...
trait Common {
    ...
    /**
     * 构造查询语句
     * @param $query     $where['goods_name|goods_brief'] =[['like','%'.$keyrowd.'%'],['like','%'.$keyrowd.'%']];
     * @param array $map $where['_string'] = 'status=1 and id=123'
     * @return mixed
     */
    public function scopeMap($query, array $map) {
        foreach ($map as $key => $val) {
            if($key == '_string'){
                $query->whereRaw($val);
            }else{
                if(!strstr($key,'&') && !strstr($key,'|')) {
                    $query->switch($key, $val);
                } else {
                    if(strstr($key,'&')){
                        $whereKey = explode('&',$key);
                        $query->where(array_combine($whereKey,$val));
                    }else{
                        $whereKey = explode('|',$key);
                        $wheres = array_combine($whereKey,$val);
                        $query->where(function($query) use($wheres){
                            $query->orParam($wheres);
                        });
                    }
                }
            }
     ...
     * @return mixed
     */
    public static function saveData($data,$type='insert',$where=null){
        switch($type){
            case 'insert':
                return parent::create($data);
                break;
            case 'update':
                return parent::map($where)->update($data);
                break;
        }
    }
}
