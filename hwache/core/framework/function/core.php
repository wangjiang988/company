...
function showMessage($msg,$url='',$show_type='html',$msg_type='succ',$is_show=1,$time=2000){
	...
	switch ($show_type){
		...
		case 'exception':
			...
			echo '<dd><p /><p /><p /><p /></dd>';
			echo '<dd><p /><p /><p /><p />Copyright 2014-'.date('Y').' Rocky, All Rights Reserved '.$lang['company_name'].'</dd>';
			echo '</dl>';
			echo '</body>';
			echo '</html>';
			exit;
 ...
 */
function Model($model = null,$table=null){
	static $_cache = array();
	...
	if (!file_exists($file_name)){
		...
		if (!class_exists($class_name)){
			...
			return $_cache[$model] = new $class_name($table);
		}
	}
}
...
/**
 * 检测FORM是否提交
 * @param bool|是否验证token $check_token 是否验证token
 * @param bool|是否验证验证码 $check_captcha 是否验证验证码
...
function write_file($filepath, $data, $mode = null){
	...
	if ($data === '') $data = '\'\'';
	$data = "<?php defined('InHG') or exit('Access Invalid!'); return ".$data."\n?>";
