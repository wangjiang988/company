...
class Model{
    ...
    public function setCountCol($col)
    {
        $this->countCol = $col;
    }
    ...
    public function getById($id){
        return $this->where([$this->get_pk() =>$id])->find();
    }
     ...
     * @param $where
     * @param $update
     * @param $old_data 该条记录的旧数据
     * @param null $operation array ['user_id'=>..,'remak'=> ..,'operation'=>...] 操作人的用户id
     * @return $update结果
     */
    public function update_with_record($where,$update,$old_data =[] ,$operation = null)
    {
        //提取出字表的更新数据
        ...
        //添加修改记录
        //操作记录
        //user_id ,type , operation 这三个字段必选，remark 可选
        $now = get_now2();
        //是否是已经存在的$operation

        if(!isset($operation['id']))
        {
            $tmp  =  explode('|',$operation['related']);
            $id   =  $tmp[1];
            ...
            $operation = [
                ...
                'related_id' => $id,
                'step'       => $operation['step']? $operation['step']  : 0,
                'type'       => isset($operation['type']) ?$operation['type']: C('operation_type')['withdraw_line'],
                'remark'     => $operation['remark'] ? $operation['remark']  : '' ,
            ];

        }

        $update_fields = array_keys($update);
        //具体某条记录的值得改变
        $details  = [];
        foreach ($update_fields as $key)
        {
            //前后值一样，不做记录
            if($old_data[$key] == $update[$key]) continue;

            $detail = [
                'table_name' => $this->get_table_name(),
                'field_name' => $key,
                'related_id' => $old_data[$this->get_pk()],
        ...
        //返回操作日志数组，主要用到id字段
        $operation_id = logger($operation,$details);
        //处理附件与操作id绑定问题
        switch ($operation['type']){
            case 12 :   //12 转入-银行转账 操作日志
                //更新该步骤的所有附件信息
                if( $this->get_table_name() !='hc_user_recharge') break;
                $model    =   Model();
                $map = [
                    'related'        =>    'hc_user_recharge|'. $old_data['ur_id'],
                    'step'           =>    $old_data['status'],
                    'is_del'         =>     0
                ];
                $map_update  = [
//                        'step'       =>  $update['status'],
                ...
                $model->table('hc_admin_operation_comment')->where($map)->update($map_update);
                break;
            case 13 :   //13  提现日志
                //更新该步骤的所有附件信息
                if( $this->get_table_name() !='hc_user_withdraw') break;
                $model    =   Model();
                $map = [
                    'related'        =>    'hc_user_withdraw|'. $old_data['ur_id'],
                    'step'           =>    $old_data['status'],
                    'is_del'         =>     0
                ];
                $map_update  = [
//                        'step'       =>  $update['status'],
        ...
        //对字表进行更新
        $new_operation = ['id'=>$operation_id];
        //更新子表并记录
        //递归
        if($son_update){
            $son_table_list  = array_keys($son_update);
            foreach ($son_table_list  as   $table_name)
            {
                $update_info = $son_update[$table_name];
                $model = Model($table_name);
                $model->update_with_record($update_info['where'],$update_info['update'],$update_info['old_data'],$new_operation);
            }
        }
     ...
     * 根据where查找list
     */
    public function getListByWhere($where, $field="*", $order='created_at desc')
    {
     ...
     * @return array
     */
    protected function setLikeWhere(array $array)
    {
        $where = [];
     ...
     * @return array|bool
     */
    protected function setStartToEnd(array $array)
    {
        $where = [];
        foreach($array as $key =>$val){
            if($val[0] !=''|| $val[1] !=''){
                ...
                if ($val[1] !='' && $val[0] ==''){
                    $where[$key] = ['elt',$val[1].$val[2]];
                }
                if ($val[0] !='' && $val[1] !=''){
                    $where[$key] = ['exp',sprintf("%s BETWEEN '%s' AND '%s'",$key,$val[0],$val[1].$val[2])];
                }
            }
        }
        return $where;
    ...
    /**
     * @param $array ['status'=>['gt',1]]
     * @return array
     */
    protected function setOperationWhere(array $array)
    {
        $where = [];
        foreach($array as $key =>$val){
	...
	public function tableInfo($table){
        ...
        //如果有是嵌套查询， 里边有表名
//        if(strpos($table," AS ")){
//            $pos =strripos($table,' AS ');
//            $table_info = substr($table,0,$pos);
//            $table_pr   = substr($table,$pos+4);
//            $this->fields[$table_pr] = $table_info;
//            return $this->fields[$table_pr];
//        }
    ...
    public function __call($method,$args) {
        if(in_array(strtolower($method),array('table','order','where','on','limit','having','group','lock','distinct','index','attr','key'),true)) {
			...
			pagecmd('setTotalNum',	$this->get_field(sprintf('count(%s) AS nc_count',$this->countCol)));
        	return $this;
    ...
    protected function parse_options($options=array()) {
        ...
        if(!isset($options['table'])){
        	...
        	foreach(explode(',', trim($options['table'],', ')) as $val){
        	    if(strripos($val,' AS ')) $tmp[] = $val;//如果有as 则不用再加 wangjiang
        		else $tmp[] = $this->getTableName($val).' AS `'.$val.'`';
        	}
...
//        如果有是嵌套查询， 里边有表名 add by wangjiang
        if(strpos($table," AS ")){
//            $pos =strripos($table,' AS ');
//            $table_info = substr($table,0,$pos);
//            $table_pr   = substr($table,$pos+4);
...
//                    $whereStr .= ' ('.$key.' '.$val[1].') ';
    ...
    protected function parseJoin($options = array()) {
        ...
        //(select order_id|SUM( CASE WHEN type = 10 THEN money ELSE -money END) as sum from car_hc_order_jiaxinbao_detail as detail  group by order_id) AS jiaxinbao
        //上述子查询，不能有逗号， 所以把| 改为,
        foreach ($table as $key => $item)
        {
            if(strpos($item,'|')){
                $table[$key] = str_replace('|',',',$item);
            }
        }
    ...
    public function insert($data,$options=array(),$replace=false) {
        ...
        //$sql = str_replace(["'uuid()'","'now()'","'NOW()'"],["uuid()","now()","NOW()"],$sql);
        // echo $sql;exit;
        return DB::execute($sql);
    }
