...
final class Csv{
    ...
    public function exportFile($displayFields,$exportList)
    {
        header("Content-type:application/vnd.ms-csv; charset=UTF-8");
        header("Content-Disposition:attachment;filename={$this->filename}.csv");
        foreach ($displayFields as $filedValue)
        {
            $filedValue = str_replace("\"","\"\"",$filedValue);
            echo "\"".$filedValue."\",";
        }
        echo base64_decode("DQo=");
        if (!empty($exportList))
        {
            foreach ($exportList as $exportValue)
            {
                foreach ($displayFields as $filed=>$tmp)
                {
                    $echoValue=str_replace("\"","\"\"",$exportValue[$filed]);
                    echo "\"".$echoValue."\",";
                }
                echo base64_decode("DQo=");
            }
