<?php defined('InHG') or exit('Access Invalid!');?>
    <div class="page finance">
        ...
        <form method="get"  id="ajax_form">
            ...
            <input type="hidden" name="back" id="back_input" value="0">
            ...
            <div class="span3">
                <span style="font-size: 50px;">已有转账</span>
            </div>
                    <?php }?>
            <?php }else{?>
            ...
            <?php if(!$output['is_operator']){?>
                <div class="span2">
                    <span style="font-size: 30px;">已有转账</span>
                </div>
                <?php }?>
            <?php }?>
            <div class="info">
                <div class="span4">
                    ...
                    <span class="val tb-type1">
                                            <input type="text" id="transfer_at" name="transfer_at" class="text wt150 date" value="">
                                        </span>
                </div>
                <div class="span4">
                    <span class="label">转账接单人：</span>
                    <span class="val"><?php echo $output['withdraw']['operation_admin_name'] ;?></span>
                </div>

                <div class="span4">
                    <span class="label">转账接单时间：</span>
                    <span class="val">
                                             <?= $output['withdraw']['operation_at']   ?>
                                         </span>
                </div>
            </div>
        </form>
        ...
        <div class="info footer">
            <a href="javascript:void(0);" style="padding-left:100px;float: left;" onclick="reject_form()">拒绝办理？</a>
            <a href="javascript:void(0);" class="button" onclick="back_form()">退单</a>
            <a href="javascript:void(0);" class="button confirm" onclick="confirm_form()">已转账，提交</a>
...
<script type="text/javascript">
    ...
    {
        parent.layer.confirm('确定不办理该笔提现申请吗？', {
            btn: ['确认','取消'] //按钮
        }, function(){
            $('#reject_input').val(1);
            ajax_form();
    ...
    function back_form()
    {
        parent.layer.confirm('确定要退出操作该提现转账吗？', {
            btn: ['确认','取消'] //按钮
        }, function(){
            $('#reject_input').val(0);
            $('#back_input').val(1);
    ...
    function confirm_form(){
        //确认转账日期已填写
        let transfer_at = $('#transfer_at').val();
        if(transfer_at ==""){
            alert('请选择转账日期');
            return false;
        }

        parent.layer.confirm('确定已完成该笔提现转账了吗？', {
            btn: ['确认','取消'] //按钮
        }, function(){
            submit_form();
            closeLayer();
