    ...
    <form id="seller_form" action="<?=url('seller_finance','save_withdraw');?>" method="post" enctype="multipart/form-data">
        ...
        <table class="table tb-type2" width="100%">
            ...
            <tr>
                ...
                <?php
                if($output['find']['kefu_confirm_status'] ==4) {
                    echo '<td colspan="2" rowspan="3"><h1 style="font-size: 30px; line-height: 60px;">已有处理</h1></td>';
                }else{
            ...
            <tr>
                <td class="required" colspan="4">收款方账号：<?=$output['find']['bank_account'];?></td>
            </tr>
            <tr>
                <td class="required" colspan="4">收款方户名：<?=$output['find']['daili_bank_name'];?></td>
            </tr>
            ...
            ?>
                <tr>
                    <td class="required" colspan="2">转账日期：<input class="date" name="recharge_confirm_at" readonly/> </td>
                    <td class="required" colspan="2">转账接单人：<?=$output['operations']['user_name'];?></td>
                    <td class="required" colspan="2">转账接单时间：<?=$output['operations']['created_at'];?></td>
                </tr>
            <?php }?>
            <!-- 备注 -->
            <tr>
            ...
            <tr class="tfoot">
                <td colspan="15" style="text-align: center;" >
                    ...
                    switch ($output['find']['kefu_confirm_status']){
                        case 0:
                            echo '<button type="button" class="button button-primary button-small" onclick="saveSubmitType(\'agree\')">批准办理</button>';
                            break;
                        case 3:
                            echo '<button type="button" class="button button-primary button-small" onclick="saveSubmitType(\'orders\')">提现接单</button>';
                            break;
                        case 4:
                            echo '<button type="button" class="button button-primary button-small" onclick="saveSubmitType(\'confirms\')">提现确认</button>&nbsp;';
                            echo '<button type="button" class="button button-primary button-small" onclick="saveSubmitType(\'returns\')">退单</button>';
                            break;
                    }
                    ?>
                    <button type="button" class="button button-primary button-small" onclick="saveSubmitType('refuse')">拒绝办理?</button>
