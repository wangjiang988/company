...
<div class="page">
    ...
    <form id="seller_form" action="<?=url('seller_finance','save_withdraw');?>" method="post" enctype="multipart/form-data">
        ...
        <table class="table tb-type2">
            ...
            <tr>
                <td width="required" colspan="6">收款路线：<?=$output['find']['bank_name']?></td>
            </tr>
            <tr>
            ...
            <?php
            if(in_array($output['find']['kefu_confirm_status'],[2,6])) {
                ?>
                <tr>
                    <td class="required" colspan="6" style=" margin-left: 5px;border-bottom: 1px #000 solid;">
                        <b>
                            <?php
                            echo ($output['find']['kefu_confirm_status']==6) ? '更正提现转账信息' : '补充提现转账信息' ;
                            ?>
                        </b>
                    </td>
                </tr>
                ...
                <tr>
                    <td colspan="6">银行凭证号：<input type="text" name="bank_voucher_code" class="txt" value="<?=$output['find']['bank_voucher_code']?>" /></td>
                </tr>
                <tr>
                    <td colspan="6">记账凭证号：<input type="text" name="accounting_voucher" class="txt" value="<?=$output['find']['accounting_voucher']?>" /></td>
                </tr>
                <tr>
                    <td colspan="6">上传凭证：<input type="file" name="recharge_voucher" class="txt" /></td>
                </tr>
                <?php
            }
            ?>
            ...
            <tr class="tfoot">
                <td colspan="15" style="text-align: center;" >
                    <?php
                    switch ($output['find']['kefu_confirm_status']){
                        case 2:
                            echo '<button type="button" class="button button-primary button-small" onclick="saveSubmitType(\'saves\')">保存</button>&nbsp;';
                            echo '<button type="button" class="button button-primary button-small" onclick="saveSubmitType(\'subSaves\')">提交补充信息</button>';
                            break;
                        case 1:
                            echo '<button type="button" class="button button-primary button-small" onclick="notSuccess()">转账未成功？</button>';
                            break;
                        case 6:
                            echo '<a class="button button-primary button-small" onclick="saveSubmitType(\'updates\')">更正转账信息</a>&nbsp;';
                            echo '<a class="button button-primary button-small" onclick="saveSubmitType(\'notUps\')">不更正转账信息</a>';
                            break;
                    }
                    ?>
