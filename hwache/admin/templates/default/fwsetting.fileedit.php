...
<div class="page">
  <div class="fixed-bar">
    <div class="item-title">
      <h3>文件设置</h3>
      <ul class="tab-base"><li><a href="index.php?act=fwsetting&op=files"><span>交车需要的文件设置</span></a></li><li><a class="current"><span>修改</span></a></li></ul>
    </div>
  </div>
  <div class="fixed-empty"></div>
  <form id="form" method="post">
    <input type="hidden" name="form_submit" value="ok" />
    <input type="hidden" name="file_id" value="<?php echo $output['file']['file_id'] ?>" />
    <table class="table tb-type2">
      ...
      <tr>
          ...
          <td><select name="car_use_type" id="car_use_type">
                  ...
                  <option value="0"
                      <?php if($file['car_use_type'] === '0') echo "selected"?>
                  >非营业个人客车</option>
                  <option value="1"
                      <?php if($file['car_use_type'] === '1') echo "selected"?>
                  >非营业企业客车</option>
                  <option value="2"
                      <?php if($file['car_use_type'] === '2') echo "selected"?>
                  >其他</option>
              </select></td>
      </tr>
      <tr>
      ...
      <tr>
          ...
          <td>
              <input type="text" class="w300" name="title" id="title_input" value="<?=$file['title']?>" />
          </td>
      </tr>
    ...
    function get_identity(car_use_type)
    {
        ...
        post('index.php?act=fwsetting&op=get_identity_by_carusetype',{'car_use_type':car_use_type})
            .then(function(res){
                ...
                {
                    let identity_id = <?=$file['identity_id']?$file['identity_id']:'0'?>;
    ...
    function get_cate(car_use_type)
    {
        post('index.php?act=fwsetting&op=get_cate_by_carusetype',{'car_use_type':car_use_type})
            .then(function(res){
                if(res.data.code=200)
                {
                    let cate_id = <?=$file['cate_id']?$file['cate_id']:'0'?>;
                    html = '<option value="">全部</option>';
                    data = res.data.data;
                    if(data){
                        for (x in data)
