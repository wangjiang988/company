...
<div class="page finance">
    ...
    <div class="container">
                    ...
                    <div class="pure-u-4-5">
                        <?php if(!empty($template['content']) && is_array($template['content'])){ ?>
                            <?php foreach($template['content'] as $k => $v){ ?>
                                <div>提现金额：￥<?=$v['range'][0]?> ~ <?=$v['range'][1]>0?$v['range'][1]:'以上'?>, 提现手续费：￥<?=$v['fee']?></div>
                            <?php } ?>
                        <?php }?>
                    </div>
                </div>
        ...
        <div class="info footer">
            ...
            <a href="index.php?act=admin_finance&op=user_add_withdraw_limit"   class="ml-50 font-14" >添加模板</a>
            <a href="index.php?act=admin_finance&op=user_change_withdraw_limit_history" class="ml-20 font-14">查看更新记录</a>
        </div>
...
<script type="text/javascript">
    ...
    function confirm_form() {
        ...
        parent.layer.confirm('确定要更新模板吗？', {
            ...
            url  = 'index.php?act=admin_finance&op=user_change_withdraw_limit'
