...
class DoJob
{
     ...
     * 投放验证码
     */
    public static function releaseVouchers($param = array())
    {
        $model = Model('hc_vouchers');
        $model->startRelease($param);
    }
     ...
     * @return mixed
     */
    public static function addVouchers($param = array())
    {
        $model = Model('hc_vouchers');
        $model->addAllVoucher($param);
        /*$time = time();
        echo "Time: {$time}, Func: ".get_class()."::".__FUNCTION__."(".json_encode($param).")\n";*/
   }
}
...
/**
* Class taskEvent 执行定时任务
*/
class taskEvent{
    public static function addTask($time=null,$object='releaseVouchers',array $param , $total=1)
    {
        Timer::dellAll();
        if($total ==1){
            Timer::add( 1, array('DoJob',$object),$param, false);
        }else{
            for($i=0;$i<$total;$i++){
                Timer::add( 1, array('DoJob',$object),$param, false);
            }
