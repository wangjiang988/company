...
\$area_array = array(\n
EOT;
        $areaData = $m->getAreaList(
            array('area_deep'=>array('in', '1,2')),
            'area_id,area_name,area_parent_id'
        );
        foreach ($areaData as $item) {
            $areaFile .= "    ".$item['area_id']."=>array('area_name'=>'".$item['area_name']."','area_parent_id'=>".$item['area_parent_id']."),\n";
        }
        $areaFile .= ");\n";

        // 写入缓存
        file_put_contents(BASE_DATA_PATH.'/area/area.php', $areaFile);
        echo 'ok';
    }
}