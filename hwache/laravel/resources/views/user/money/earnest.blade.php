    ...
    @if($useEarnest)
      <div class="wapper has-min-step">
        ...
        <div class="box-border pay-box">
          <form action="{{ route('user.money.postEarnest') }}" method="post" name="payform">
            {{ csrf_field() }}
      ...
      <div class="wapper has-min-step">
        <ul class="price-ul">
          <li><span>>>应付诚意金：￥{{ $earnest }}</span></li>
          <li>可用余额：￥{{$money}}<span class="juhuang">（查看）</span></li>
          <li class="last">可用余额：<span class="juhuang">￥{{$userMoney}}</span><label>（余额不足，请充值到可用余额）</label></li>
          <div class="clear"></div>
        </ul>
