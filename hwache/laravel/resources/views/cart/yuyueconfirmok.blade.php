    ...
    <div class="container pos-rlt content r-pdi" ms-controller="item">
        ...
        <div class="wapper has-min-step" style="overflow: visible;">
            ...
            <div class="radio i-checks">
                <label class="fs14">
                    <input <?php if(isset($user_choose['songche']) && $user_choose['songche']=='自己开回'){echo 'checked';} ?> type="radio" name="songche" value="自己开回" disabled>
                    <i></i>
                    自己开回（无送车服务费）
                </label>
            </div>
            ...
            <div class="radio i-checks">
                <label class="fs14">
                    <input <?php if(isset($user_choose['songche']) && $user_choose['songche']=='代驾送车'){echo 'checked';} ?> type="radio" name="songche" value="代驾送车" disabled>
                    <i></i>
                    代驾送车  
                </label>
            </div>
            ...
            <div class="radio i-checks">
                <label class="fs14">
                    <input <?php if(isset($user_choose['songche']) && $user_choose['songche']=='板车运输送车'){echo 'checked';} ?> type="radio" name="songche" value="板车运输送车" disabled>
                    <i></i>
                    板车运输送车
                </label>
            </div>
            @endif
            <table class="tbl songche" style="<?php if(isset($user_choose['songche']) && $user_choose['songche']!='代驾送车'){echo 'display: none';} ?>">
             @if(isset($pdiReply['daijia']['choose']))
                <tr>
                    <td width="50%">
            ...
            <table class="tbl songche" style="<?php if(isset($user_choose['songche']) && $user_choose['songche']!='板车运输送车'){echo 'display: none';} ?>">
            @if(isset($pdiReply['transport']['choose']))
                <tr>
                    <td width="50%">
           ...
           @if(count($baoxian)>0)
	            ...
	            <div class="radio i-checks">
	                <label class="fs14">              
	                    @if(isset($v['choose']) && $v['choose'] == 'Y')
		                    <input type="checkbox" name="baoxian[{{$k}}]" value="Y" disabled <?php if(isset($user_choose['baoxian'])&& in_array($k,$user_choose['baoxian'])){echo 'checked';}?>>
		                    <i></i>
            ...
            @if(count($pdiReply['project_ok']>0))
            	@foreach($pdiReply['project_ok'] as $k => $v)
		            <div class="radio i-checks">
		                <label class="fs14">
		                    <input type="checkbox" name="project_ok[]" value="{{$v['name']}}" disabled <?php if(isset($user_choose['project_ok'])&& in_array($v['name'],$user_choose['project_ok'])){echo 'checked';}?>>
		                    <i></i>
            ...
            <!-- 可以办理的服务或者赠品 -->
             @if(count($pdiReply['service_ok']>0))
            	@foreach($pdiReply['service_ok'] as $k => $v)
            		<p class="psr fs14">
            		 	<input type="checkbox" name="service_ok[]" value="{{$v}}" disabled <?php if(isset($user_choose['service_ok'])&& in_array($v,$user_choose['service_ok'])){echo 'checked';}?>>
            ...
            <div class="box">
                <div class="box-inner box-inner-def tbl">
                    <p class="center">
                        ...
                        <input type="type" value="已确认提交" class="btn btn-s-md btn-danger fs16">
                    </p>
                </div>
            </div>
        </div>
