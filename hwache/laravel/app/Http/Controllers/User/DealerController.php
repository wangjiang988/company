...
class DealerController extends Controller
{
    ...
    public function memberInfo($type = 'show')
    {
        ...
        if ($type == 'show') {
            ...
            $view['kefu'] = array(
                'name'        => '小白',
                'email'       => 'service@hwache.com',
                'mobilephone' => '13885885885',
                'phone'       => '0512-68782885-8002',
            );
    ...
    public function modifyPasswordCheck()
    {
        ...
        if ($answer != $member['seller_answer']) {
            return json_encode(array('error_code' => 4, 'error_msg' => '问题回答不正确'));
        }
...
//				$updata = array('dl_step'=>$nextStep);
    ...
    public function sendEmail($email, $do = 'send_email_to_change_mobile')
    {
        ...
        if ($do == 'send_email_to_change_mobile') {
            ...
            $url = $_ENV['_CONF']['config']['www_site_url'] . '/dealer/changemobile/verify_email_to_change_phone?data=' . Crypt::encrypt($data);
        } elseif ($do == 'send_email_to_change_email') {
            $data['verify'] = 1;
            $subject = trans('common.verify.email');
            $tpl = '_layout.email_send_to_change_email';
            $url = $_ENV['_CONF']['config']['www_site_url'] . '/dealer/changeemail/verify_email_to_change_email?data=' . Crypt::encrypt($data);
        } elseif ($do == 'send_email_to_check_orgin_email') {
            $data['verify'] = 1;
            $subject = trans('common.verify.email');
            $tpl = '_layout.email_send_to_change_email';
            $url = $_ENV['_CONF']['config']['www_site_url'] . '/dealer/changeemail/verify_email_to_check_email?data=' . Crypt::encrypt($data);
        } else {
            return false;
        }
        //邮件发送模板
