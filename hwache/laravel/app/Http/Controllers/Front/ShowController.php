...
class ShowController extends Controller
{
     ...
     * 详情页面
     */
    public function getShow($urls, $buytype)
    {
        ...
        //车辆详细
        $result['buytype'] = ($buytype == 1) ? 1 : 0; //默认为个人用车
        ...
        //随车工具
        $result['annexs'] = HgAnnex::getAnnex($result['brand_id']);
        //原厂选装件
        $result['originals'] = $this->getXzjType($id);
        //上牌服务
    ...
    public function getCarCalc(Baojia $baojia, $id, $area_id, $buytype)
    {
        ...
        $newData['pailiang'] = round($pailiang / 1000, 1);
    ...
    private function getRegions($city)
    {
        ...
        $areas['tips'] = Area::where('area_id', $areas['area_id'])->select('tips','special_file')->first()->toArray();
        return $areas;
    }
     ...
     * 选装件分类
     */
    public function getXzjType($id)
    {
        $datas = HgBaojiaXzj::getBaojiaXzj($id)->toArray();
        if (count($datas) > 0) {
            $xzj['rpo_sum'] = 0;
