<?php namespace App\Com\Hwache\Amap;
/**
 * 高德地图JSAPI扩展类
 *
...
class Jsapi
{
     ...
     * @return              返回结果
     */
    public function getLocal($data=array()) {
        ...
        if(!empty($this->sig)) {
            $url .= '&sig='.$this->_getSig($data);
        }
        return $this->_curlPostGet($url, '', 0);
    }
     ...
     */
    public function getAround($data=array()) {
    ...
    public function getPolygon($location, $radius, $data=array()) {
     ...
     */
    private function _getSig(array $data) {
        $tmp_arr = array();
        foreach($data as $k => $v) {
            $tmp_arr[$k] = $k.'='.$v;
        }
     ...
     * @return 返回获取到的数据流
     */
    private function _curlPostGet($url, $data=array(), $method=1) {
        $ch = curl_init();
        $opt = array(
