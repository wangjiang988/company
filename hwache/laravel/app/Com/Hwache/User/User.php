...
class User
{
    ...
    public function sendEmail($email,$do='reg')
    {
		...
		if($do=='reg'){
			...
			$url = $_ENV['_CONF']['config']['www_site_url'].'/user/memberSafe/verify_email/'. Crypt::encrypt($data);
			...
			$url = $_ENV['_CONF']['config']['www_site_url'].'/user/memberSafe/verify_email_to_change_phone/'. Crypt::encrypt($data);
			...
			$url = $_ENV['_CONF']['config']['www_site_url'].'/user/memberSafe/verify_change_passwd/'. Crypt::encrypt($data);
		}
		//邮件发送模板
        return Mail::send(
            $tpl,
