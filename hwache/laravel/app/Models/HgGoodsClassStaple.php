...
class HgGoodsClassStaple extends  Model
{
   ...
   public static function getCarSearch($daili_id,$dealer_id,$gc_name)
   {
     ...
     */
    public static function delStaple($dl_id, $staple_id)
    {
        if ($staple_id > 0) {
            return DB::transaction(function () use ($dl_id, $staple_id) {
                $row = DB::table('goods_class_staple')->where('staple_id',$staple_id)->first();
                // 常用车型表改状态
