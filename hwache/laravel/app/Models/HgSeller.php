...
class HgSeller extends Model {
    ...
    {
    	$seller = self::leftJoin('member','seller.seller_id','=','member.member_id')->where('seller.seller_id','=',$seller_id)->first();
    	return $seller;
    }
    //经销商登录检测；
    ...
    public static function checkLoginByPhone($phone){
    	$seller = self::leftJoin('member','seller.seller_id','=','member.member_id')->where('member.member_mobile','=',$phone)->first();
    	return $seller;
    }
}
