       ...
       if (empty($ids) || !is_array($ids)) {
          ...
          foreach ($filenames as $key => $value) {
            // 文件数量
               ...
               // 文件场合
               $cate=HgOrderFiles::where('order_num','=',$order_num)->where('isself','=',1)->where('title','=',$value->title)->lists('cate')->toArray();
          ...
          $filenames=self::select('title')->whereIn('id',$ids)->distinct()->get();
            ...
            // 文件数量
               ...
               $files2[$key]['num']=$file_count;
               // 文件场合
               $cate=HgOrderFiles::where('title','=',$value->title)->whereIn('id',$ids)->lists('cate')->toArray();
