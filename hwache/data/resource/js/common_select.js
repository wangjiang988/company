...
function getDeptUser(_value,_target){
    var url = '/index.php?act=admin&op=dept_user';
    $.getJSON(url, {'dept':_value}, function(data){
        if (data)
        {
            var _options = '<option value="">选择员工</option>';
            if (data.length > 0)
            {
                //$(_target).html(_options);
				$(data).each(function(_key,_item){
					_options += '<option value="'+_item.admin_truename+'">'+_item.admin_truename+'</option>';
				});
				$(_target).html(_options);
            }else{
                $(_target).html('<option value="">选择员工</option>');
			}
        }
    });
}