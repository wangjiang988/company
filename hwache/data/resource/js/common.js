...
function login_dialog(){
    ...
    CUR_DIALOG = ajax_form('login','登录','/index.php?act=finance&&op=ajax_set_status',360,1);
...
function refresh_workspace(){
    ...
    workspace.contentWindow.location.reload();
}
 ...
 * @returns {boolean}
 */
function valid(ele_id,reg){
    if (!reg.test($("#"+ele_id).val())) {
        return false;
    }
    return true;
}
...
function submit_ajax_form(url,json,return_url)
{
...
function submit_ajax_form2(url,form_id,return_url)
{
    let json= $('#'+form_id).serializeJSON();
 ...
 * @param height
 */
function show_dialog(title, html_id, width, height){
    let html = $(html_id).html();
    layer.open({
        type: 1,
        // title:"修改退款周期",
        title:title,
