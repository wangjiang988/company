...
class memberModel extends Model {
	...
	public function addMember($param) {
		...
		//$member_info['member_passwd']		= crypt(trim($param['member_passwd']),'$1$rasmusle$');
        $member_info['member_passwd']       = $param['member_passwd'];
...
// 	}
	...
	//获取售方加信宝信息列表
    public function getMemberAccountList($field='*',$where=null,$order='asc',$page=10){
        $where['member.member_state'] = 1;
		if(!isset($where['seller.seller_phone']))
			$where['seller.seller_phone'] = ['exp','LENGTH(trim(seller.seller_phone)) >1'];
        $order  = 'hc_daili_account.freeze_deposit '.$order;

        $return = $this->table('member,seller,hc_daili_account')
            ->join('left,left')
            ->on('member.member_id = seller.member_id, member.member_id=hc_daili_account.d_id')
    ...
    {
        $model = Model('hc_daili_account');
        return $model->table('hc_daili_account')->field('SUM(freeze_deposit) as sum_freeze_deposit')->where($where)->find();
    }

    //获取售方加信宝信息列表
    public function getMemberAccount($where=[]){
        $where['member.member_state'] = 1;
        $return = $this->table('member,seller,hc_daili_account')
            ->join('left,left')
            ->on('member.member_id = seller.member_id, member.member_id=hc_daili_account.d_id')
    ...
    //查询加信宝订单列表
    public function getMemberJiaxinbaoLogs ($where=[])
    {
        $return = $this->table('hc_order,member,(select order_id|sum( case when type = 10 then money else -money end) as sum from car_hc_order_jiaxinbao_detail as detail where detail.role=2  group by order_id) AS jiaxinbao')
            ->field('member.member_id,hc_order.id,hc_order.created_at,jiaxinbao.sum')
            ->join('left,left')
            ->on('member.member_id=hc_order.seller_id,hc_order.id=jiaxinbao.order_id')
