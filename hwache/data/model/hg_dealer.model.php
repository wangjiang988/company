...
class hg_dealerModel extends Model {

    public function __construct(){
        parent::__construct('hg_dealer');
    }
    ...
    public function getDealerInfo($condition, $field = '*') {
        return $this->table('hg_dealer,goods_class')
            ->field($field)
            ->join('left')
     ...
     * @param string $order
     */
    public function getDealerList($condition = array(), $field = '*', $page = 0, $order = 'd_id desc') {
        return $this->table('hg_dealer,goods_class')
            ->field($field)
            ->join('left')
            ->on('hg_dealer.d_brand_id=goods_class.gc_id')
     ...
     * @return int
     */
    public function getDealerCount($condition) {
	 ...
	 */
	public function addDealer($param) {
		if(empty($param)) {
			return false;
		}
		$result	= Db::insert('hg_dealer',$param);
		if($result) {
			return Db::getLastId();
		} else {
			return false;
	 ...
	 * @return	array 数组格式的返回结果
	 */
	public function updateDealer($param,$d_id) {
		if(empty($param)) {
			return false;
		}
		$update		= false;
		//得到条件语句
		$condition_str	= " d_id='{$d_id}' ";
		$update		= Db::update('hg_dealer',$param,$condition_str);
		return $update;
	}
	/**
	 * 经销商登录检查
...
// 	}
	 ...
	 * 查询经销商总数
	 */
	public function countDealer($condition){
		//得到条件语句
		$condition_str	= $this->getCondition($condition);
