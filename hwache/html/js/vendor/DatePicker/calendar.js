...
My97DP.prototype = {
	init: function() {
		...
		//开始时间
		$stdt = this.startDate = new DPDate();
		//结束时间
		$edt = this.endDate = new DPDate();
		//console.log($edt )
	...
	_fd: function() {
		...
		for (_ = 1, F = C; _ < 7; _++) {
			...
			for ($ = 0; $ < 7; $++) {
				...
				if (K.M == D) {
					...
					if (K.compareWith($sdt, "d") == 0) {
						...
						if (K.compareWith($tdt, "d") == 0) {
							...
							H = ($dp.highLineWeekDay && (0 == (B + $) % 7 || 6 == (B + $) % 7) ? "" : "");
						}
					}
					//E = ($dp.highLineWeekDay && (0 == (B + $) % 7 || 6 == (B + $) % 7) ? "WwdayOn" : "WdayOn");
					E = ($dp.highLineWeekDay && (0 == (B + $) % 7 || 6 == (B + $) % 7) ? "" : "");
				} else {
					if ($dp.isShowOthers) {
						J = true;
						//H = "WotherDay";
				...
				//如果当前输出的时间在mintime和endtime之间
				//并且在startime之后的15天
				//K当前时间
				//console.dir("_:"+_)
				//console.log("K.M:"+K.M)
				//console.log("$edt.M:"+$edt.M)
				//console.log("$stdt.M:"+$stdt.M)

				if ( 

					K.y == $edt.y && K.y == $stdt.y &&
					K.M <= $edt.M && K.M >= $stdt.M &&
					K.d <= $edt.d && K.d >= $stdt.d
	...
	_fy: function(A, $) {
		...
		_.a(">←</td><td class='menu' onmouseover=\"this.className='menuOn'\" onmouseout=\"this.className='menu'\" onmousedown=\"hide($d.yD);$d.yI.blur();\">×</td><td ");
	...
	_fillQS: function($) {
		...
		if (!$) {
			A.a('<div style="float:right;cursor:pointer" onclick="hide($d.qsDivSel);">×</div>');
		}
		A.a("</td></tr>");
...
function _tab(_) {
	...
	if ($dp.enableInputMask && !$OPERA && !$dp.readOnly && $c.currFocus == $dp.el && (N >= 48 && N <= 57)) {
		var J = $dp.el,
			...
			O = 0,
			K = 0,
			R, E = /yyyy|yyy|yy|y|MM|M|dd|d|%ld|HH|H|mm|m|ss|s|WW|W|w/g,
