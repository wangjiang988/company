...
class store_settingControl extends BaseSellerControl {
    ...
    public function store_dailiOp() {
        ...
        $model_daili_dealer = Model('hg_daili_dealer');
        /**
         * 查询代理的经销商列表
         */
        // 查询条件
        $condition = array('dl_id'=>$_SESSION['member_id']);
        // 查询字段
        $field = '*';
        // 排序
        ...
        // 查询数据
        $daili_dealer_list = $model_daili_dealer->getDailiDealerList($condition, $field, 10, $order);
        foreach ($daili_dealer_list as $k => $v) {
            $tmp = $this->_getDealerInfo($v['d_id']);
            if(!empty($tmp)) {
                $daili_dealer_list[$k]['d_name'] = $tmp['d_name'];
                $daili_dealer_list[$k]['d_areainfo'] = $tmp['d_areainfo'];
            } else {
                $daili_dealer_list[$k]['d_name'] = '';
                $daili_dealer_list[$k]['d_areainfo'] = '';
            }
        }//var_dump($daili_dealer_list);exit;

        Tpl::output('daili_dealer_list',$daili_dealer_list);
        Tpl::output('page',$model_daili_dealer->showpage());
        self::profile_menu('store_daili');
        Tpl::output('menu_sign','store_setting');
        Tpl::output('menu_sign_url','index.php?act=store_setting&op=store_setting');
        Tpl::output('menu_sign1','store_slide');
        /**
	...
	*/
	function delOp(){
		...
		showMessage('删除成功','index.php?act=store_setting&op=store_daili');
	}
    /**
     * 代理商信息查询
     */
    private function _getDealerInfo($id) {
        $model_dealer = Model('hg_dealer');
        $info = $model_dealer->getDealerInfo(array('d_id'=>$id), 'd_name,d_areainfo');
        if(!empty($info)) {
            return $info;
        } else {
            return false;
    ...
    public function add_dealerOp() {
        ...
        $condition = array();
        // 查询字段
        $field = 'd_id,d_name,d_yy_place,d_areainfo';
        // 排序
        $order = trim($_GET['search_sort']);
        if (empty($order)) {
            $order = 'd_id desc';
        ...
        // 查询数据
        $dealer_list = $model_dealer->getDealerList($condition, $field, 10, $order);

        Tpl::output('dealer_list',$dealer_list);
        Tpl::output('page',$model_dealer->showpage());
        // 页面模板
        Tpl::showpage('store_daili_add_dealer_form');
    }

    /**
     * 选择经销商
     */
    public function select_dealerOp() {
        $model_daili_dealer = Model('hg_daili_dealer');

        if(chksubmit()) {
            $data = array(
                'dl_id'         => $_SESSION['member_id'],
                'd_id'          => trim($_POST['d_id']),
                'dl_jc_lngx'    => trim($_POST['jc_lngx']),
                'dl_jc_lngy'    => trim($_POST['jc_lngy']),
                'dl_jc_place'   => trim($_POST['jc_place']),
            );
            // 商业保险
            if($_POST['baoxian']) {
                $data['dl_baoxian']      = 1;
            ...
            // 交车联系人信息
            $data['dl_jc_data']     = serialize($_POST['data']);
            $r = $model_daili_dealer->addDailiDealer($data);
            if($r) {
                showMessage('成功添加经销商', 'index.php?act=store_setting&op=store_daili');
            } else {
            ...
            // 查询是否已经添加该经销商
            $if_add = $model_daili_dealer->getDailiDealerCount(
                array('dl_id'=>$_SESSION['member_id'],'d_id' => $id)
            );
            if($if_add) {
                showMessage('已经添加过该经销商，无需重复添加', 'index.php?act=store_setting&op=store_daili');
            }
            $model_dealer = Model('hg_dealer');
            ...
            // 页面模板
            Tpl::showpage('store_daili_select_dealer_form');
        }
    }
    ...
    public function ajaxgetdealerOp() {
        $id = isset($_GET['id'])
            ...
            : 0;
        if(empty($id)) {
            echo json_encode(array('code'=>0,'msg'=>'经销商参数错误！'));exit;
        }
        // 获取指定id经销商的数据信息
        $condition = array(
            'd_id' => $id,
    ...
    private function profile_menu($menu_key='') {
        ...
        $menu_array = array(
            ...
            // 增加经销商代理选择经销商功能
            3=>array('menu_key'=>'store_daili','menu_name'=>'经销商管理','menu_url'=>'index.php?act=store_setting&op=store_daili'),
