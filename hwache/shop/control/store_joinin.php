...
class store_joininControl extends BaseHomeControl {
     ...
     * 选择经销商类型（个人，公司）
     */
    public function hgswitchOp() {
        Tpl::showpage('store_joinin_apply.hgswitch');
    }

    /**
     * 选择经销商类型（个人，公司）
     */
    public function hgstep1Op() {
    	if(!empty($_POST)){
    	}
    	$jxs = Model('hg_dealer')->getDealerList();
    	Tpl::output('jxs', $jxs);
        Tpl::output('step', 'hgstep2');
        Tpl::output('sub_step', 'hgstep1');
