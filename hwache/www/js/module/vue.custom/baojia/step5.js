define(function (require,exports,module) {
    /*var addTpl = {
      template: '<div v-cloak class="def-add" :style="{display:isAdd ? \'table-row\' : \'none\'}">A custom component!</div>',
    }*/
    var step6 = new Vue({
        el: '#vue',
        delimiters : ['${', '}'],
        ...
        methods: {
            ...
            ,
            save:function(bj_id){
                if (!this.canAdd || this.elm.count == "") {
                    this.isError = true
                    return
                }
                var _this = this
                //console.log(bj_id,_this.elm.id,_this.elm.value,_this.elm.count)
                ...
                $.ajax({
                     ...
                     data: {
                        id:0,
                        bj_id:bj_id,
                        other_id:_this.elm.id,//选中的费用名称id
                        other_name:_this.elm.value,
                        other_price:_this.elm.count,
                     ...
                     success: function(data){
                         ...
                         /*if(data.error_code==0){
                             ...
                             //自定义了一个：'服务器繁忙，请稍后再试！'
                             $("#tip-error").hcPopup({content:''+data.error_msg ? '服务器繁忙，请稍后再试！' : data.error_msg+''})
                         }else{
                            //window.location.reload()
            ...
            ,
            modify:function(index,id,bj_id){
                var _this = this
                ...
                $.ajax({
                     ...
                     data: {
                        id:id,
                        bj_id:bj_id,
                        other_id:_this.switchs[index].id,//选中的费用名称id
                        other_name:_this.switchs[index].value,
                        other_price:_this.switchs[index].countDisp,
                        _token:$("meta[name=csrf-token]").attr('content')
                     }
                     ,
                     dataType: "json",
                     success: function(data){
                         ...
                         //if (data.error_code==0) {
                            //require("module/common/hc.popup.jquery")
                            //$("#tip-error").hcPopup({content:'操作成功！'})
            ...
            del:function(event){
                ...
                this.delOption.dealer = _del.attr("bj")
    ...
    //var data = step6._data
    //console.log(data)
    Vue.component('my-component', {
        //template: '<tr v-cloak class="def-add" :style="{display:isAdd ? \'table-row\' : \'none\'}"><td>xxx</td></tr>'
        template: '<div v-cloak class="def-add" :style="{display:isAdd ? \'table-row\' : \'none\'}">A custom component!</div>',
        data:function(){
            return step6._data
        }
